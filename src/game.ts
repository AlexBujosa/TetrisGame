import { ctx, Color, SquarePiece, LPiece, LInvPiece, ZPiece, ZInvPiece, TPiece, LinePiece, TypePiece, Piece } from "./classes/piece.js";
const widthGame = 506;
...
let generatePiece = (choosePiece: number): Piece => {
    var randomNumber = generate();
    var colorPiece: Color = RandomColor(randomNumber);
    var piece: Piece;
    switch (choosePiece) {
...
let confirmXRotation = (piece: Piece): boolean => {
...
let confirmYRotation = (piece: Piece): boolean => {
    var next_DegrePos = (piece.degrePos + 1) % 4;
    for (let i = 0; i < piece.arrPos[next_DegrePos].length; i++) {
...
let getPieceSquareX = (piece: Piece): number[] => {
    let setNum = new Set<number>();
...
function RandomColor(num: number): Color {
    const arrColor: Color[] = [Color.BLUEDARK, Color.BLUELIGHT, Color.GREEN, Color.ORANGE, Color.PURPLE, Color.RED, Color.YELLOW];
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.code == 'ArrowRight' && posXPieceSquare.indexOf(20) == -1) {
        ...
        var oldPiece: Piece = pieces.shift()!;
        var randomNumber = generate()
        var piece = generatePiece(randomNumber)
        pieces.push(piece);
        rotate = 0;
